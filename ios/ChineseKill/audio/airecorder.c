          ...
          const AudioStreamPacketDescription *packet_desc)
{
  ...
  if (buffer->mAudioDataByteSize > 0) {
    if (recorder->callback) {
      recorder->callback(recorder->usrdata, buffer->mAudioData, buffer->mAudioDataByteSize, recorder->dlgdata);
    }
...
int
airecorder_start(struct airecorder *recorder, const char* wavpath, airecorder_callback callback, const void* usrdata, int callback_interval, const void* dlgdata)
{
  int rv = -1;
  ...
  rv = AudioQueueSetProperty(recorder->audio_queue, kAudioQueueProperty_EnableLevelMetering, &enabledLevelMeter, sizeof(UInt32));
  if (rv != noErr){
    goto end;
  }
...
double
airecorder_getVolume(struct airecorder * recorder)
{
  double volume = 0;
  if (recorder == NULL || recorder->is_running == 0) {
    goto end;
  ...
  int rv = AudioQueueGetProperty(recorder->audio_queue, kAudioQueueProperty_CurrentLevelMeterDB, &levelMeter, &levelMeterSize);
  if (rv != noErr){
    volume = 0;
  }else{
